...
function loadMore(res) {
    res.forEach(function (bot) {
        ...
        <div class="card">
            ...
            <a href="/bots/${bot.botid}" class="button small">Ver informações do bot</a>
        </div>`
...
function search() {
    ...
    if (cards) {
        ...
        if (cardsVisible === 0) {
            document.getElementById('searchMore').innerHTML = `Nenhum bot encontrado. Você gostaria de <a href="/bots/search/?q=${s}">procure todos os bots</a>?`;
            document.getElementById('searchMore').style.display = "block";
        } else {
            document.getElementById('searchMore').innerHTML = `Você gostaria de <a href="/bots/search/?q=${s}">procura todos os bots</a>`
            document.getElementById('searchMore').style.display = "block";
        }
